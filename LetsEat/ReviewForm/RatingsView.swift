...
//  LetsEat
//
//  Created by עלאא דאהר on 09/07/2020.
...
class RatingsView: UIControl {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for index in 0...totalStars {
            let value = cellWidth * CGFloat(index) + cellWidth / 2.0
            let center = CGPoint(x: value + 1, y: rect.size.height / 2.0)
    ...
    func updateRating(with value: CGFloat) {
        if value != self.rating && value >= 0 && value <= CGFloat(totalStars) {
            self.rating = value
            setNeedsDisplay()
        }
    }
